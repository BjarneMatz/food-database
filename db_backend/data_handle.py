...
class Database:
    ...
    def get_value(self, key: str) -> Union[Any, None]:
        ...
        Args:
            key (str): Schlüssel des Wertes in der Datenbank

        Returns:
            Union[Any, None]: Wert des Schlüssels oder None, falls der Schlüssel nicht existiert
        """
        data = self._read()
        try:
            value = data[key]
    ...
    def set(self, key: str, value: Any) -> None:
        ...
        """
        data = self._read()
        Log.log(f"Schreibe {key} in die Datenbank.")
        data[key] = value
        self._write(data)
    ...
    def delete_key(self, key: str) -> None:
        ...
        try:
            ...
            raise KeyError(
                f"Der Schlüssel {key} existiert nicht in der Datenbank.")

    def delete_db(self, confirm: bool = False) -> None:
        """Löscht die Datenbank-Datei und alle Daten vom System.

        Args:
            confirm (bool, optional): Bestätigung der Löschung. Standard ist False.
        """
        if confirm:
            os.remove(self.database_path)
        else:
            raise PermissionError(
                "Die Datenbank kann nur mit dem Parameter confirm=True gelöscht werden.")
